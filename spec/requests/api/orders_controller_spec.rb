  ...
  context 'with authorization token' do
    describe 'GET/api/orders' do
      ...
      it 'JSON body response contains a list of orders' do
        expect(json['orders'][0].keys).to match_array(%w[id created_at total_cost
                                                         food_items user])
      ...
      it 'each order contains list of food_items attributes' do
        expect(json['orders'][0]['food_items'][0].keys).to match_array(%w[name course price])
      ...
      it 'each order contains user attributes' do
        expect(json['orders'][0]['user'].keys).to match_array(%w[name email])
