...
RSpec.describe OrdersController, type: :controller do
  context 'unathenticated user' do
    ...
    describe 'PATCH /update' do
      it 'redirects to login page with alert flash' do
        patch :update,
              params: { daily_menu_id: daily_menu, id: order, order: attributes_for(:order) }
        expect(response).to redirect_to(new_user_session_url)
        expect(flash[:alert]).to be_present
      end
    end
  ...
  context 'authorized user' do
    ...
    before do
      ...
      order.order_items.first.food_item = first_course
      order.save
    end
    ...
    describe 'POST /create' do
      context 'without order_items' do
        let(:attrib) { attributes_for_list(:food_item, 3, food_item_id: '') }
        it 'renders :new' do
          post :create, params: { daily_menu_id: daily_menu,
                                  order:         { user_id:                user,
                                                   order_items_attributes: attrib } }
          expect(response).to render_template(:new)
        end
      ...
      context 'with valid order_items' do
        let(:attrib) { attributes_for(:order_item, food_item_id: first_course) }
    ...
    describe 'PATCH /update' do
      context 'valid data' do
        ...
        let(:updated_attr) { attributes_for(:order_item, food_item_id: new_first_course.id) }
        let(:delete_attr) { order.order_items[0].attributes.merge!(_destroy: true) }
        it 'redirects to daily_menu path' do
          patch :update, params: { daily_menu_id: daily_menu, id: order, user_id: user,
                                  order: { order_items_attributes: [updated_attr,
                                                                    delete_attr] } }
